...
def load_data(prefix, i):
    ...
    p_LR_path = f'{dir_data}/output/LR/p{prefix}_data/pmax_LR_P{prefix}_data{i}.mat'
    p_HR_path = f'{dir_data}/output/HR/p{prefix}_data/pmax_HR_P{prefix}_data{i}.mat'
    ux_LR_path = f'{dir_data}/output/LR/p{prefix}_data/ux_LR_P{prefix}_data{i}.mat'
    uy_LR_path = f'{dir_data}/output/LR/p{prefix}_data/uy_LR_P{prefix}_data{i}.mat'
    uz_LR_path = f'{dir_data}/output/LR/p{prefix}_data/uz_LR_P{prefix}_data{i}.mat'
    sk_LR_path = f'{dir_data}/input/sk{prefix}_data/skull_patch{prefix}_data{i}.mat'
...
def load_unforeseen_data(prefix, i):
    ...
    p_LR_path = f'{dir_data}/output/LR/sk{prefix}_data/pmax_LR_P{prefix}_data{i}.mat'
    p_HR_path = f'{dir_data}/output/HR/sk{prefix}_data/pmax_HR_P{prefix}_data{i}.mat'
    ux_LR_path = f'{dir_data}/output/LR/sk{prefix}_data/ux_LR_P{prefix}_data{i}.mat'
    uy_LR_path = f'{dir_data}/output/LR/sk{prefix}_data/uy_LR_P{prefix}_data{i}.mat'
    uz_LR_path = f'{dir_data}/output/LR/sk{prefix}_data/uz_LR_P{prefix}_data{i}.mat'
    sk_LR_path = f'{dir_data}/input/sk_data_old/test_sk{prefix}_data/skull_patch{prefix}_data{i}.mat'
...
del sk_data
Vx_train, Vx_valid = train_test_split(Vx_data, test_size = 0.16666, random_state = 1)
del Vx_data 
Vy_train, Vy_valid = train_test_split(Vy_data, test_size = 0.16666, random_state = 1)
del Vy_data
Vz_train, Vz_valid = train_test_split(Vz_data, test_size = 0.16666, random_state = 1)
del Vz_data
...
def save_dataset_hdf5(ds, filename):
    with h5py.File(filename, 'w') as h5f:
        for i in range(len(ds)):
            group = h5f.create_group(str(i))
            item = ds[i]
            for j, tensor in enumerate(item[:-1]):  # Assuming the last item is the label
